...
from .gammagram import gtgram_strides


def spectrogram(wave, fft_length=None, fs=44000, window_time=0.01,
                hop_time=0.01):
    ...
    fft_length : int, divisible by 2; optional.
    ...
    np.ndarray, dtype=complex.
        Shape (Tau, fft_length/2 + 1)
        Where `Tau = 1 + floor((sound_length - fft_length)/hop_bins))`.
    ...
    if fft_length is None:
        fft_length = _default_fft_length(window_time, fs)

    sound_length = wave.shape[0]
    window_bins, hop_bins, _ = gtgram_strides(fs, window_time, hop_time, 0)
    win = _spectrogram_window(fft_length, window_bins)

    t = 0

    # pre-allocate output array
    n_time_bins = 1 + int(np.floor((sound_length - fft_length)/hop_bins))
    spectrogram = np.zeros((n_time_bins, (1 + fft_length//2)), np.dtype(complex))

    for b in range(0, sound_length - fft_length, hop_bins):
      u = win * wave[b : b + fft_length]
      fft = np.fft.fft(u)
      spectrogram[t, :] = fft[0 : (1 + fft_length//2)]
      t = t + 1
...
def _default_fft_length(window_time, fs):
...
def fft_gammagram(wave, fs=44000, window_time=0.01, hop_time=0.01,
    ...
    gt_weights, _ = fft_weights(
        fft_length, fs, channels, f_min, f_max=fs/2, maxlen=(fft_length/2 + 1)
        )

    sgram = spectrogram(wave, fft_length, fs, window_time, hop_time)
    result = np.abs(sgram).dot(gt_weights) / fft_length
