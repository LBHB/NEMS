...
Lives in nems.tools (next to nems.tools.arrays, its only nems dependency)
rather than nems.models: it is a plain data container, not a model, and
importing it from nems.models pulled the whole model package into
nems.preprocessing.split, creating an import cycle
(preprocessing.split -> models.dataset -> models/__init__ -> models.LN ->
models.base -> nems.visualization -> nems.preprocessing). A backward-compat
shim remains at nems.models.dataset.
