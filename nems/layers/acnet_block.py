...
class ResAdd(Layer):
    ...
    Computes `gamma * res_scale * main + skip @ shortcut`, where `shortcut`
    is a learnable (Cin, Cout) projection **initialized to zero** and `gamma`
    is a learnable scalar initialized to 1 -- so at construction the skip
    contributes nothing and the block is its main path at full strength.
    Matches ACNet's `ResBlock_CNN1d_W_NL_v1` residual sum exactly
    (`PT_EncMdl_helpers_v2.py`): `shortcut` there is a zero-initialized
    `nn.Linear`, applied to the block's original (pre-FIR) input.
    ...
    def evaluate(self, main, skip):
        ...
        shortcut_out = np.tensordot(skip, shortcut, axes=(1, 0))
    ...
    def as_tensorflow_layer(self, **kwargs):
        ...
        class ResAddTF(NemsKerasLayer):
            def call(self, inputs):
                main, skip = inputs
                shortcut_out = tf.tensordot(skip, self.shortcut, axes=[[2], [0]])
